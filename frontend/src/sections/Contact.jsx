...
import { WHATSAPP, MAPS_EMBED } from "../lib/data";
import api from "../lib/api";

const INTERESTS = ["Software", "AI Solutions", "Cloud", "Learning", "Internship"];

export default function Contact() {
  const [form, setForm] = useState({ name: "", email: "", company: "", interest: "Software", message: "" });
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setStatus("success");
      setForm({ name: "", email: "", company: "", interest: "Software", message: "" });
    } catch (err) {
        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16">
          <div>
            ...
            <p className="mt-6 text-muted text-lg leading-relaxed max-w-md">
              Whether you're launching a product or leveling up your team, we'd love to hear from you.
            </p>

            <div className="mt-8 space-y-3">
              {[
                { icon: Mail, label: "hello@3capstech.com" },
                { icon: Phone, label: "+91 90000 00000" },
                { icon: MapPin, label: "Hyderabad, India" },
              ].map((c) => (
                <div key={c.label} className="flex items-center gap-3 text-muted">
                  <span className="h-10 w-10 grid place-items-center rounded-full glass text-accent"><c.icon size={17} /></span>
                  {c.label}
                </div>
              ))}
            </div>
